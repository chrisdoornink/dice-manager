...
export const useGameInitialization = ({
  ...
  const isValidPosition = useCallback(
    (
      ...
      avoidTerrainTypes: string[] = ["water"]
    ) => {
    ...
    (
      ...
      switch (strategy) {
        ...
        case "edges":
          ...
          candidatePositions = validPositions
            .filter((p) => {
          ...
          while (selectedPositions.length < count && candidatePositions.length > 0) {
            ...
            for (const candidate of candidatePositions) {
              ...
              for (const selected of selectedPositions) {
                const distance =
                  Math.abs(candidate.q - selected.q) + Math.abs(candidate.r - selected.r);
            ...
            candidatePositions = candidatePositions.filter(
              (p) => !(p.q === bestPosition.q && p.r === bestPosition.r)
            );
          }
      ...
      candidatePositions = candidatePositions.filter(
        (pos) => !isPositionOccupied(pos, occupiedPositions)
      );
  ...
  const getEnemyPositions = useCallback(
    (
      ...
      count: number,
      strategy: EnemyPlacementStrategy = "far",
      occupiedPositions: GridPosition[] = []
    ): GridPosition[] => {
        ...
        case "surrounding":
          ...
          const surroundingRingPositions = validPositions.filter((pos) => {
            ...
            const minDist = Math.min(
              ...playerPositions.map((pPos) => Math.abs(pos.q - pPos.q) + Math.abs(pos.r - pPos.r))
            );
          ...
          candidatePositions =
            surroundingRingPositions.length > 0
        ...
        case "far":
          ...
          candidatePositions = validPositions.sort((a, b) => {
            ...
            const minDistA = Math.min(
              ...playerPositions.map((p) => Math.abs(a.q - p.q) + Math.abs(a.r - p.r))
            );
            const minDistB = Math.min(
              ...playerPositions.map((p) => Math.abs(b.q - p.q) + Math.abs(b.r - p.r))
            );
  ...
  const initializeNewGame = useCallback(
    (
      playerStrategy: PlacementStrategy = "center",
      enemyStrategy: EnemyPlacementStrategy = "far"
    ) => {
      if (allGridPositions.length > 0) {
        ...
        const newPlayerEntities: PlayerEntity[] = [
          ...
          },
          {
            ...
            entityType: initializeWithHealth(playerUnitTypes, "warrior"),
          },
          {
            id: "mage-1",
          ...
          const unoccupiedPositions = allGridPositions
            .filter((pos) => {
              ...
              return (
                terrain &&
                terrain.type !== "water" &&
                !occupiedPositions.some((occupied) => occupied.q === pos.q && occupied.r === pos.r)
              );
            })
  ...
  const initializeWithConfig = useCallback(
    (config: {
      playerPlacement?: PlacementStrategy;
      enemyPlacement?: EnemyPlacementStrategy;
